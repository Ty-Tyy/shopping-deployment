...
const fetchJewelleryProducts = async () => {
  try {
    ...
    const jewelleryProducts = response.data.filter((item) => item.type?.type_name === 'Jewellery')
    products.value = markRaw(jewelleryProducts)
  } catch (error) {
    console.error('Error fetching jewellery’s product data:', error)
  }
}
/**
 * Add Product to Cart
...
onMounted(() => {
  ...
  cartStore.dispatch('cart/loadCart')
})
</script>
