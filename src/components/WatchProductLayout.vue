...
const fetchWatchProducts = async () => {
  try {
    ...
    const watchProducts = response.data.filter((item) => item.type?.type_name === 'Watch')
    products.value = markRaw(watchProducts)
  } catch (error) {
    console.error('Error fetching watch’s product data:', error)
  }
}
/**
 * Add Product to Cart
