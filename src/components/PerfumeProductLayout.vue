              ...
              <div class="flex flex-col md:flex-row justify-between md:items-center flex-1 gap-6">
                <div class="flex flex-row md:flex-col justify-between items-start gap-2">
                  <div>
                    ...
                    <div class="text-lg font-medium mt-2">{{ product.product_name }}</div>
                  </div>
                </div>
                <div class="flex flex-col md:items-end gap-8">
                  <span class="text-xl font-semibold">${{ product.product_price }}</span>
                  <Button
...
const fetchPerfumeProducts = async () => {
  try {
    ...
    const perfumeProducts = response.data.filter((item) => item.type?.type_name === 'Perfume')
    products.value = markRaw(perfumeProducts)
  } catch (error) {
    console.error('Error fetching perfume’s product data:', error)
  }
}
